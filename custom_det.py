    ...
    for path, im, im0s, _, s in dataset:
        ...
        if len(pred[0]) >1:
            for i, det in enumerate(pred):  # per image
                ...
                split = txt_path.split(os.path.sep)[-1].split('&')
                leftx, topy = int(split[-2]) , int(split[-1])
                # print(txt_path,leftx, topy)
                single_patch_info.append((leftx, topy ))
                if len(det):
                    ...
                    single_patch_info.append(lales_list)
                all_results.append(single_patch_info)
        else:
            for i, det in enumerate(pred):  # per image
                seen += 1
                if webcam:  # batch_size >= 1
...
def bbox_inference(image, boxes):
    ...
    dl = torch.utils.data.DataLoader(
        ds, batch_size=16, num_workers=2, shuffle=False, drop_last=False
    )
    results = []
    results = np.zeros((len(ds), 224, 224)).astype(np.uint8)
    print(image.shape, len(ds), results.shape)
    count = 0
    for idx, data in enumerate(dl):
        with torch.no_grad():
            with autocast():
                ...
                final_result = (seg_results > 0.5).int().to('cpu').numpy().astype(np.uint8)
                # print(final_result.max(),final_result.min())
        for sub_item in final_result:
            results[count] = sub_item
            count += 1
    ...
    for raw_image, img_path in zip(img_list, all_images):
        ...
        if min(shape[:2]) > 1000:
            ...
            for file in all_results:
                ...
                confs = []
                leftx, topy  = file[0]
                leftx, topy = leftx / shape[1], topy / shape[0]
                for res in file[1]:
                    cls, x, y, w, h, conf = res
                    ...
                    if min(xmin, ymin, xmax, ymax) > 5 and max(xmin, ymin, xmax, ymax) < 995:
                       ...
                       # boxes.append([xmin, ymin, xmax, ymax,conf,0])

                        confs.append(conf)
                box_set.extend(boxes)
